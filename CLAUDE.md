...
- Email body is a stack of blocks (paragraph, heading, image, button, divider). Paragraph text keeps three inline constructs (blank line, `**bold**`, `[text](https://…)`); everything else is escaped. `{{first_name}}` is always available; automations may add more variables per recipient.
